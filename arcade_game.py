...
This game demonstrates some of the more advanced features of Arcadem including:
...
class ArcadeGame(arcade.Window):
    ...
    def __init__(self, width: float, height: float, title: str):
        ...
        # Call the super class in the method
        super().__init__(width, height, title)

        # Set up a timer to create new coins (in seconds)
    ...
    def setup(self):
        ...
        arcade.set_background_color(color=arcade.color.ALICE_BLUE)

        # Set up the player
        sprite_image = ASSETS_PATH / "images" / "monster.png"
        self.player = arcade.Sprite(
            filename=sprite_image, 
            center_x = WIDTH // 2,
            center_y = HEIGHT // 2,
        )
        #

        # Schedule the spawn of new coins after a specific interval
        arcade.schedule(
            function_pointer = self.add_coin, 
            interval= self.coin_countdown
        )

        # Load your coin collision sound
        self.coin_pickup_sound = arcade.load_sound(
            ASSETS_PATH / "sounds" / "handle_coins.wav"
        )

    def add_coin(self, dt:float):
        """Add a new coin to the screen, reschedule the timer

        Arguments:
            dt {float} -- Time since last call (unused) 
        """

        # Create a new coin that spawns randomly between margins of 20 px of the window
        coin_image = ASSETS_PATH / "images" / "coin.png"
        ...
        # Decrease the time between the coin appearances, but only if there are fewer
        # then three coins on the screen
        if len(self.coins) < 3:
